            ...
            @if(in_array($acc,["adrian","ted"]))
                ...
                @if(!empty($final_money) && is_array($final_money))
                    <div class="row">
                        <div class="col-xs-6 col-md-6">
                            <div class="input-group ">
                                <span class="input-group-addon">统计总工资:{{@$final_money['']}}</span>
all_mone
                                <span class="input-group-addon">统计总课程收入:{{@$all_lesson_price}}</span>
                                <span class="input-group-addon">统计模拟总工资:{{@$all_money_simulate}}</span>
                                <span class="input-group-addon">统计模拟课程收入:{{@$all_lesson_price_simulate}}</span>
                                <span class="input-group-addon">统计总工资差别:{{@$all_money_different}}</span>
                                <span class="input-group-addon">统计课程收入差别:{{@$all_lesson_price_different}}</span>
                            </div>
                        </div>
                    </div>
                @endif
