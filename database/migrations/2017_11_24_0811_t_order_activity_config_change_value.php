...
class TOrderActivityConfigChangeValue extends Migration
{
    /**
     * Run the migrations.
    ...
    public function up()
    {
        ...
        Schema::create('db_weiyi.t_order_activity_config', function (Blueprint $table){
            ...
            t_field($table->string('grade_list')->nullable(),"适配年级");
            t_field($table->integer('open_flag')->nullable(),"是否手动开启活动 0:关闭 1:正式开启 2:测试开启");
            t_field($table->integer('can_disable_flag')->nullable(),"是否手动开启活动 1:可以手动关闭 0:不可手动关闭");
            t_field($table->string('period_flag_list')->nullable(),"分期试用");
            t_field($table->string('contract_type_list')->nullable(),"合同类型");
            ...
            t_field($table->string('discount_json',4096)->nullable(),"打折活动封装的json字符串");
