	...
	$.ajax({type: "post",url: _urlbase+"recommend.php",dataType: 'json',
		...
		success: function(data) {
			...
			for(var i=0;i<data.recommend.length;i++){
				var ap = $("<tr><td>"+(i+1)+".<a href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxbefb2cda01c3f409&redirect_uri=http://ts.whytouch.com/wxforward2.php&response_type=code&scope=snsapi_base&state="+data.recommend[i].uuid+"Zleo1v1()"+window.firsttag+"**"+data.recommend[i].tag+"#wechat_redirect'>"+delExtension(data.recommend[i].filename)+"</a></td></tr>");
