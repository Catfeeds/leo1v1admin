...
//初始化区域图的对象
  ...
  if(this.$editable){
    ...
    Y=ev.y-t.top+this.parentNode.scrollTop;
        This.addNode( Math.floor( new Date().getTime()/1000),{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType});
    This.$max++;
    });
    //划线或改线时用的绑定
  ...
  initGroup:function(width,height){
    ...
    if(e.data.inthis.$ghost.css("display")=="none"){
      ...
      var color=["red","yellow","blue","green"];
        e.data.inthis.addArea( Math.floor( new Date().getTime()/1000),
  ...
  initWorkForNode:function(){
    ...
    this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
      ...
      if(lineStart&&!This.$mpTo.data("p")){
          ...
          This.addLine( Math.floor( new Date().getTime()/1000),tmp);
        This.$max++;
      }
      else{
        if(lineStart){
