// SWITCH-TO:   ../../old_admin/template/appoint
$(function(){
    $.ajax({
        url: '/appoint/get_appoint_course',
        type: 'POST',
        data: {
        },
        dataType: 'json',
        success: function(result) {
            if (result['ret'] == 0) {
                load_package_list(result['course_list']);
            } else {
    ...
    var load_package_list = function(package_list){
        ...
        for(var i=0; i<package_list.length; i++) {
            var package_str = '<tr>' +
                '<td >' + package_list[i]['package_name'] +"<br/>" + package_list[i]['packageid'] + '</td>' +
                '<td >' + package_list[i]['subject'] + '</td>' +
                    '<td >' +  package_list[i]['grade'].replace(/,/g,",<br/>") + '</td>' +
                '<td >' + package_list[i]['price'] + '</td>' +
                '<td >' + package_list[i]['package_type'] + '</td>' +
                '<td >' + package_list[i]['package_tags'] + '</td>' +
                '<td >' + package_list[i]['tag_type'] + '</td>' +
                '<td >' + package_list[i]['package_deadline'] + '</td>' +
                '<td data-packageid="'+ package_list[i]['packageid'] +
                '" data-type="' + package_list[i]['package_type'] + '">'+
                '<button class="btn fa fa-info td-info " href="javascript:;" title="查看信息"></button>'+
                '<button class="btn fa fa-edit opt-package-edit" href="javascript:;" title="修改信息"></button>'+
                '<button class="btn fa fa-photo opt-package-pic" href="javascript:;" title="设置图片"></button>'+
                '<button class="btn fa fa-comment opt-package-intro" href="javascript:;" title="设置简介"></button>'+
                '<button class="btn fa fa-file-powerpoint-o opt-package-outline" href="javascript:;" title="设置大纲"></button>'+
                '<button class="btn fa fa-users opt-package-class" href="javascript:;" title="设置班级"></button>'+
                '<button class="btn fa fa-cny opt-package-price" href="javascript:;" title="设置价格"></button>'+
                '<button class="btn fa fa-clock-o opt-package-users" href="javascript:;" title="设置购买的时间和人数"></button>'+
                '<button class="btn fa fa-slideshare opt-package-open-courseid" href="javascript:;" title="设置公开课id"></button>'+
               // '<button class="btn fa fa-user-plus opt-package-buy" href="javascript:;" title="get package bought"></button>'+
                '<button class="btn fa fa-trash-o opt-package-delete" href="javascript:;" title="删除记录"></button>'+
                '<button class="btn fa fa-tag opt-package-tag-type" href="javascript:;" title="设置 标签"></button>'+
                '</td></tr>';
            $('#opt-package-list').append(package_str);
        }
    ...
    var dp_set_package_pic = function(packageid, package_pic) {
        ...
        var th = setTimeout(function(){
            custom_upload('opt-upload-package-pic', 'opt-upload-package-pic-parent', g_UPLOAD_DOMAIN_URL, set_modify_package_pic);  
            clearTimeout(th);
        }, 1000);
    };
