    ...
    var show_plot=function( id_name,label, data_list,color,title_funcion ) {
        ...
        $("#"+id_name).bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                ...
                // $("#tooltip").html( title_funcion(data_item) ).css({top: item.pageY+5, left: item.pageX+5}).fadeIn(200);
                $("#"+mark).html( title_funcion(data_item) ).css({top: item.pageY+5, left: item.pageX+5}).show();
            } else {
                $("#"+mark).show();
            }
        });
        ...
        $("#id"+id_name).bind("plotclick", function (event, pos, item) {
            ...
            $("#tooltip").html( title_funcion(data_item) ).addClass('show');
        });
    }

    show_plot("id_contract_money","金额",money_list,null,function(data_item){
        return "时间:"+data_item[0]+ "<br/>" +"金额:"+data_item[1]+ "<br/>";
    });
