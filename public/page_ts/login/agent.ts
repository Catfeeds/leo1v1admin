...
$(function(){
    ...
    function login_fun(){
        ...
        if (!check_phone() || !check_password() || !check_seccode()) {
            $('#verify_image').attr('src', '/login/get_verify_code?r='+Math.random());
            return;
        }
        ...
        $.ajax({
            'url':'/login/agent_login',
            'type': 'POST',
            'data': {'phone':phone,'password':password,'seccode':seccode},
            // 'data': {'phone':phone,'password':password,'seccode':seccode,'remember':remember},
            'dataType': 'jsonp',
            success: function(data) {
                if (data['ret'] == 0) {
                    ...
                    if (to_url ){
                        ...
                        window.location.href=  "/agent_info";
                    }
                } else {
