...
function checkRich(){
	if(document.getElementsByClassName('rich_media_tool').length>0){
		...
		rd.style[window.aj] = "scale(4)";
		rd.style[window.aj2] = "0 0";
		rd.style.marginTop="200px";
		rd.className = "box2";
		
		rd.innerHTML="<div class='inner'><span style='line-height:25px'>百度微演示,PPT轻松发手机。</span><table class='rank_list' cellspacing='20'></table></div>";
		window.ac.appendChild(rd);
		//checkWxOpenid();
		//recommendready();
	}
}
...
function getRecommend(){
	$.ajax({type: "post",url: "http://ts.whytouch.com/recommend.php",dataType: 'json',
		...
		success: function(data) {
			...
			for(var i=0;i<data.recommend.length;i++){
				var ap = $("<tr><td>"+(i+1)+".<a href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxbefb2cda01c3f409&redirect_uri=http://ts.whytouch.com/wxforward2.php&response_type=code&scope=snsapi_base&state="+data.recommend[i].uuid+"()"+window.firsttag+"**"+data.recommend[i].tag+"#wechat_redirect'>"+delExtension(data.recommend[i].filename)+"</a></td></tr>");
...
function getComment(){
	$.ajax({type: "post",url: "http://ts.whytouch.com/comment.php",dataType: 'json',
		data:{articleid:window.firsttag,stype:"getcomment"},
		error: function(data) {
...
function subComment(){
	...
	if(getCookie("whytouch_token_openid")!=""){
		$.ajax({type: "post",url: "http://ts.whytouch.com/comment.php",dataType: 'json',
			data:{articleid:window.firsttag,openid:getCookie("whytouch_token_openid"),content:str,stype:"subcomment"},
			error: function(data) {
				alert("系统正忙，请稍后再试！");
				closesub();
			},
...
function SettingComment(){
	...
	cd.innerHTML="<img src='http://ts.whytouch.com/images/icon_edit.png' /><a href='javascript:void(0);' onclick='showsub();'>写留言</a><table class='tbcomment' id='tbcomment' border='0'></table><div class='box2 tips'><p>评论内容只代表网友观点,与[微演示]立场无关!。</p></div><div style='clear:both;'></div>";
	...
	//评论未关注公众号提示div
	$('body').append("<div id='showqrcode' style='display:none;position:fixed;width:300px;height:400px;left:50%;top:50%;margin-left:-150px;margin-top:-200px;border-radius:15px;background-color:#eaeaea;z-index:3;'><div style='width:200px;margin-left:50px;margin-top:20px;'><label style='word-wrap:break-word;word-break:break-all;'>关注我们微演示官方公众号您将会获得显示头像以及昵称的特权。<br>[长按二维码识别加关注]</label></div><img src='' width='200px' style='margin-left:50px;margin-top:150px;'/><div class='discuss_btn_wrp close'><a class='btn btn_close btn_discuss btn_disabled' href='javascript:;' onclick='document.getElementById(\"showqrcode\").style.display=\"none\";'>关闭</a></div></div>");
}
function isWeiXin(){
