/// <reference path="../../page_ts/other.d.ts" />
...
(function($, window, document,undefined) {
    ...
    Cselect_dlg_ajax.prototype = {
            ...
            $.each(me.options.filter_list,function (i,row_item){
                ...
                $.each(row_item,function(j,item){
                    ...
                    if (item.type=="select" ) {
                        ...
                        if(item.default_selected && item.value != ""){
                            $input.val(item.default_selected );
                        }
        ...
        show_display:function( ) {
            ...
            if(!me.options.is_lru_show){
                $dlg_form = $('<div class="row">'+
                              '    <div class="col-xs-12 col-md-12 ajax_list">'+
                              '    </div>'+
                              '    <div class="lru_list" style="display:none">'+
                              '    </div>'+
                              '</div>');
            ...
            if (me.options.onshown ) {
                me.options.onshown( dlg.getModalDialog() );
            }
    ...
    //在插件中使用对象
    $.fn.admin_select_dlg_ajax_second = function(options) {
        //创建的实体
        var select_dlg_ajax_second = new Cselect_dlg_ajax(this, options);
        //调用其方法
        select_dlg_ajax_second.bind();
        if(! select_dlg_ajax_second.element_is_input ){
            select_dlg_ajax_second.show_select();
        }else{
            select_dlg_ajax_second.show_display();
        }
        return  select_dlg_ajax_second;
    };
