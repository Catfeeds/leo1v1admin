(function($, window, document,undefined) {
    ...
    var Cdiv_th= function(ele, opt) {
        ...
        var date_type_str="th_id_date_type";
        var opt_date_type_str="th_id_opt_date_type";
        var start_time_str="th_id_start_time";
        var end_time_str="th_id_end_time";


        if ( field_index>0 ) {
            date_type_str="th_id_date_type_"+ field_index;
            opt_date_type_str="th_id_opt_date_type_"+ field_index;
            start_time_str="th_id_start_time_"+ field_index;
            end_time_str="th_id_end_time_"+ field_index;
        }
        var input_width= 100;
...
'                        <span id="id_date_span2" >-</span>'+
        ...
        '                            <button  type="submit" class="btn  btn-warning " id="id_th_date_post"><i class="fa    fa-search "></i></button>'+
            '                        </div>'+
...
'                        <span  style="width:100% ;opacity:0;"></span>'+
            ...
            +' </div>'
            +' </div>';
        var $th=$("#"+ this.options.th_input_id);
        var title=$.trim($th.find("span").text());
        var old_title=title;
        $th.data("title",title);

        var display_title= old_title+"<span  class=\"opt-sort\"> </span>"+
            "<br/>"+ this.options.start_time +"~"  +
        "<br/>"+ this.options.start_time +"" ;
        ...
        //排序 放进来
        html_obj.find(".a-title .opt-sort" ).append( $th.find(".td-sort-item" ));
        ...
        html_obj.find(".dropdown-menu" ) .on ("click",function(){
            return false;
        } );
      ...
      id_end_time.datetimepicker({
        ...
        $ele.find("#id_th_date_post" ).on("click",function(){
            //set to main

            var field_index=  me.options.field_index;
            ...
            var main_id_start_time=$main_ele.find("#"+start_time_str) ;
            var main_id_end_time=$main_ele.find("#"+end_time_str) ;
            var main_id_opt_date_type=$main_ele.find("#"+opt_date_type_str);
            var main_id_date_type=$main_ele.find("#"+date_type_str);
            main_id_start_time.val( id_start_time.val() );
            main_id_end_time.val( id_end_time.val() );
            main_id_opt_date_type.val( id_opt_date_type.val() );
            main_id_date_type.val( id_date_type.val() );
            me.options.onQuery();
...
'                        <span  style="width:100% ;opacity:0;"></span>'+
    ...
    $.fn.select_date_range  = function(options) {
        ...
        if (options.th_input_id ) {
            new Cdiv_th(this, options);
        }
        return  select_dlg;
    };
