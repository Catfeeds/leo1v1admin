...
function load_data(){
    ...
    $.reload_self_page ( {
		...
		check_first_revisite_time_count : $('#id_check_first_revisite_time_count').iCheckValue(),
...
$(function(){
    ...
    if ( g_args.check_first_revisite_time_count ) {
        $("#id_check_first_revisite_time_count").iCheck("check");
    }
    ...
    $.each( g_data_ex_list,function(i,item){
        if (item["title"] !="全部") {
		    ...
		    first_revisite_time_list.push([ item["title"], item["first_revisite_time_count"]>0?item["first_revisite_time_count"]:0 ]);
		    test_lesson_list.push([ item["title"], item["test_lesson_count"]>0?item["test_lesson_count"]:0 ]);
            var call_old_count= (item["call_count"]? item["call_count"]:0)- (item["first_revisite_time_count"]?item["first_revisite_time_count"]:0);
    ...
    var show_plot=function( ) {
        ...
        if ($("#id_check_first_revisite_time_count").iCheckValue()  ) {
            plot_data_list.push(
            {
                data: first_revisite_time_list,
                lines: { show: true
                         , lineWidth: 0.3},
                label: "消耗例子数"
            });
    ...
    $(".id_require_test_lesson_each").on("click",function(){
	    var date_str=get_row_date_query_str(this);
	    $.wopen("/seller_student/test_lesson_list?date_type=1&"+date_str+"quire_user_type=-1&userid=-1&teacherid=-1&confirm_flag=-1&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-3");
    });

    $(".id_test_lesson_each").on("click",function(){
        var date_str=get_row_date_query_str(this);
	    $.wopen("/seller_student/test_lesson_list?date_type=4&"+date_str+"&require_user_type=-1&userid=-1&teacherid=-1&confirm_flag=-1&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-3");         
    });

    $(".id_test_lesson_count_succ_each").on("click",function(){
	    var date_str=get_row_date_query_str(this);
        $.wopen("/seller_student/test_lesson_list?date_type=4&"+date_str+"&require_user_type=-1&userid=-1&teacherid=-1&confirm_flag=-1&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-4");
    });

    $(".id_test_lesson_count_fail_need_money").on("click",function(){
	    var date_str=get_row_date_query_str(this);
        $.wopen("/seller_student/test_lesson_list?date_type=3&"+date_str+"&require_user_type=-1&userid=-1&teacherid=-1&confirm_flag=3&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-3");
    });

    $(".id_seller_test_lesson_count").on("click",function(){
	    var date_str=get_row_date_query_str(this);
        $.wopen("/seller_student/test_lesson_list?date_type=4&"+date_str+"&require_user_type=0&userid=-1&teacherid=-1&confirm_flag=-1&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-3"); 
    });

    $(".id_seller_test_lesson_count_succ").on("click",function(){
        var date_str=get_row_date_query_str(this);
        $.wopen("/seller_student/test_lesson_list?date_type=4&"+date_str+"&require_user_type=0&userid=-1&teacherid=-1&confirm_flag=-1&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-4");
    });

    
    $(".id_seller_test_lesson_count_fail_need_money").on("click",function(){
	 var date_str=get_row_date_query_str(this);
        $.wopen("/seller_student/test_lesson_list?date_type=3&"+date_str+"&require_user_type=0&userid=-1&teacherid=-1&confirm_flag=3&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-3");
    });

    $(".seller_test_lesson_count_fail_not_need_money" ).on("click",function(){
        var date_str=get_row_date_query_str(this);
        $.wopen("/seller_student/test_lesson_list?date_type=4&"+date_str+"&require_user_type=-1&userid=-1&teacherid=-1&confirm_flag=2&page_count=undefined&phone=&from_type=-1&st_application_nick=&origin=&grade=-1&subject=-1&origin_ex=&status=-3");
    });
