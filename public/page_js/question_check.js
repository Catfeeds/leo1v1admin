...
$(function(){
    ...
    var reload_data=function( grade,subject,adminid, start_time,end_time ,note_id, page_num,url){
        var data = null;
        if (!url){
			url  = "/question/question_list_check_for_js";
            data = {
                "grade"         : grade
                ,"subject"      : subject 
                ,"adminid"      : adminid 
                ...
                ,"note_id"      : note_id 
                ,"page_num"     : page_num
                ,"check_userid" : g_args.check_userid 
		...
		do_ajax(url, data, function(result){
            ...
            $.each( ret_list, function(i,item){
                html_str+=' <tr  class="opt-edit-record">	' +
                    ...
                    '                    <td style="display:none;" >'+item.check_user+'</td>'+
    ...
    if (  path_name == "/question/question_list_check"  ) {
    ...
    var question_editor=$("#id_question_editor" ).admin_question_editor({
        ...
        "onSave":function(opt_type) {
            if (opt_type=="add" ){
                ...
                reload_data(g_args.grade, g_args.subject, g_args.adminid, g_args.start_time, g_args.end_time ,g_args.note_id,   current_page_num );
            }
        }
    });
    ...
    //选中第一条
    reload_data(g_args.grade, g_args.subject, g_args.adminid, g_args.start_time, g_args.end_time ,g_args.note_id, g_args.page_num );
