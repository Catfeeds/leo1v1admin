...
class t_lesson_info extends \App\Models\Zgen\z_t_lesson_info
{
    ...
    public function get_need_set_lesson_end_list(  ){
        ...
        $sql = $this->gen_sql("select lessonid,userid,teacherid,courseid,lesson_type,lesson_num,lesson_end from %s ".
    ...
    public function lesson_record_server_list($page_num,$record_audio_server1 ) {
        ...
        $sql=$this->gen_sql_new(
            "select lessonid, record_audio_server1, lesson_start, lesson_end, userid,teacherid"
...
lesson_type in (0,1) "
    ...
    public function get_confirm_lesson_list($start_time,$end_time) {
        $sql=$this->gen_sql("select l.assistantid ,sum(lesson_count) as lesson_count,count(*) as count, count(distinct l.userid ) as user_count from  %s  l, %s s ".
                            " where  l.userid=s.userid  and is_test_user=0 and lesson_start >=%s and lesson_start<%s  and lesson_status =2 and confirm_flag not in (2,3)  and lesson_type in (0,1,3)"
    ...
    public function get_confirm_lesson_list_user($page_num, $start_time,$end_time,$assistantid) {
        ...
        $sql=$this->gen_sql_new("select s.assistantid, s.userid ,sum(lesson_count) as lesson_count,count(*) as count from  %s  l, %s s ".
    ...
    private function get_wx_teacher_str($start_time,$end_time,$type){
        ...
        case 2:
            ...
            $str=" l.lesson_status=1 and l.tea_attend=0 and l.wx_come_flag=0 and l.lesson_type<1000";
            break;
        case 3:
            $lesson_time_str = $lesson_start_str;
    ...
    public function get_lesson_info($lessonid){
        $sql = $this->gen_sql_new("select lessonid,teacherid,assistantid,userid,lesson_type,lesson_start,lesson_end,lesson_count,"
                                  ." teacher_money_type,level,already_lesson_count,tea_attend,stu_attend,tea_rate_time,"
    ...
    public function get_teacher_test_person_num_list_subject( $start_time,$end_time){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid,l.teacherid) person_num,count(l.lessonid) lesson_num,l.subject "
                                  ." ,count(distinct c.userid,c.teacherid,c.subject) have_order"
                                  ." from %s l "
                                  ." left join %s tss on tss.lessonid = l.lessonid"
                                  ." left join %s tq on tq.require_id = tss.require_id"
    ...
    public function get_kk_teacher_test_person_num_list_subject( $start_time,$end_time){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid,l.teacherid) person_num,count(l.lessonid) kk_num,l.subject "
    ...
    public function get_change_teacher_test_person_num_list_subject( $start_time,$end_time){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid,l.teacherid) person_num,count(l.lessonid) change_num,l.subject "
    ...
    public function get_kk_teacher_test_person_num_list( $start_time,$end_time,$subject=-1,$grade_part_ex,$teacherid_list=[]){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid) kk_per_num,count(l.lessonid) kk_num,l.teacherid "
                                  ." ,count(distinct c.userid,c.teacherid,c.subject) kk_order"
                                  ." from %s l "
                                  ." left join %s tss on tss.lessonid = l.lessonid"
                                  ." left join %s tq on tq.require_id = tss.require_id"
    ...
    public function get_kk_teacher_test_person_num_list_total( $start_time,$end_time,$subject=-1,$grade_part_ex,$teacherid,$teacher_subject,$identity,$tea_subject,$qz_flag,$tea_status,$teacher_account,$fulltime_flag){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid,l.teacherid) kk_per_num,count(l.lessonid) kk_num "
                                  ." ,count(distinct c.userid,c.teacherid,c.subject) kk_order"
                                  ." from %s l "
                                  ." left join %s tss on tss.lessonid = l.lessonid"
                                  ." left join %s tq on tq.require_id = tss.require_id"
    ...
    public function get_change_teacher_test_person_num_list( $start_time,$end_time,$subject=-1,$grade_part_ex,$teacherid_list=[]){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid) change_per_num,count(l.lessonid) change_num,l.teacherid "
                                  ." ,count(distinct c.userid,c.teacherid,c.subject) change_order"
                                  ." from %s l "
                                  ." left join %s tss on tss.lessonid = l.lessonid"
                                  ." left join %s tq on tq.require_id = tss.require_id"
    ...
    public function get_change_teacher_test_person_num_list_total( $start_time,$end_time,$subject=-1,$grade_part_ex,$teacherid,$teacher_subject,$identity,$tea_subject,$qz_flag,$tea_status,$teacher_account,$fulltime_flag=-1){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid,l.teacherid) change_per_num,count(l.lessonid) change_num"
                                  ." ,count(distinct c.userid,c.teacherid,c.subject) change_order"
                                  ." from %s l "
                                  ." left join %s tss on tss.lessonid = l.lessonid"
                                  ." left join %s tq on tq.require_id = tss.require_id"
    ...
    public function get_tea_month_list($start,$end,$teacher_ref_type,$teacher_type=0,$teacher_money_type,$level){
        ...
        $sql = $this->gen_sql_new("select t.teacherid,if(t.realname='',t.nick,t.realname) as tea_nick,t.subject,t.create_time,"
                                  ...
                                  ." t.idcard,t.bank_city,t.bank_province,"
    ...
    public function get_research_test_lesson_info_list($subject,$grade,$start_time,$end_time){
        ...
        $sql = $this->gen_sql_new("select count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid,l.teacherid) all_lesson"
                                  ." from %s l "
                                  ." left join %s tss on l.lessonid = tss.lessonid"
                                  ." left join %s c on "
                                  ." (l.userid = c.userid "
    ...
    public function get_research_test_lesson_info_list_by_grade($subject,$grade,$start_time,$end_time){
        ...
        $sql = $this->gen_sql_new("select count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid,l.teacherid) all_lesson,l.grade"
                                  ." from %s l "
                                  ." left join %s tss on l.lessonid = tss.lessonid"
                                  ." left join %s c on "
                                  ." (l.userid = c.userid "
    ...
    public function get_qz_test_lesson_info_list($qz_tea_arr,$start_time,$end_time){
        ...
        $this->where_arr_add_time_range($where_arr,"l.lesson_start",$start_time,$end_time);
        $sql = $this->gen_sql_new("select count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid) all_lesson,t.teacherid,t.realname"
                                  ." from %s l "
                                  ." left join %s tss on l.lessonid = tss.lessonid"
                                  ." left join %s t on l.teacherid = t.teacherid"
                                  ." left join %s c on "
    ...
    public function get_subject_order_list_new($start_time,$end_time){
        ...
        $this->where_arr_add_time_range($where_arr,"l.lesson_start",$start_time,$end_time);
        $sql = $this->gen_sql_new("select count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid,l.teacherid) all_lesson,l.subject"
                                  ." from %s l "
                                  ." left join %s tss on l.lessonid = tss.lessonid"
                                  ." left join %s c on "
                                  ." (l.userid = c.userid "
    ...
    public function get_success_test_lesson_list_new_total($start_time,$end_time,$subject,$grade_part_ex,$teacherid,$teacher_subject,$identity,$tea_subject,$qz_flag,$tea_status,$teacher_account,$fulltime_flag=-1){
        ...
        $sql = $this->gen_sql_new("select count(distinct c.userid,c.teacherid,c.subject) order_number,count(distinct l.lessonid,l.teacherid) success_lesson"
                                  ." from %s l "
                                  ." left join %s tss on l.lessonid = tss.lessonid"
                                  ." left join %s c on "
                                  ." (l.userid = c.userid "
    ...
    public function get_train_lesson($page_num,$start_time,$end_time,$teacherid=-1,$lesson_status=-1,
        ...
        $sql = $this->gen_sql_new("select l.lessonid,l.teacherid,t.realname as tea_nick,lesson_start,lesson_end,lesson_type,"
                                  ...
                                  ." left join %s t2 on tl.userid=t2.teacherid and t2.train_through_new=1"
                                  ." where %s"
    ...
    public function get_all_teacher_test_lesson_info($start_time,$end_time,$grade_part_ex=-1,$subject=-1,$train_through_new=-1){
        ...
        $this->where_arr_add_time_range($where_arr,"l.lesson_start",$start_time,$end_time);
        $sql =$this->gen_sql_new("select t.teacherid,t.realname,count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid,l.teacherid) all_lesson,t.train_through_new_time  ".
                                 " from %s l ".
                                 " left join %s t on l.teacherid=t.teacherid".
                                 " left join %s tss on l.lessonid = tss.lessonid".
                                 " left join %s c on ".
    ...
    public function get_interview_teacher_first_lesson_info($start_time,$end_time){
        ...
        $this->where_arr_add_time_range($where_arr,"lesson_start",$start_time,$end_time);
        $sql = $this->gen_sql_new("select m.uid,count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid) person_num "
                                  ." from %s l left join %s t on l.teacherid = t.teacherid"
    ...
    public function get_interview_teacher_first_lesson_info_subject($start_time,$end_time){
        ...
        $this->where_arr_add_time_range($where_arr,"lesson_start",$start_time,$end_time);
        $sql = $this->gen_sql_new("select l.subject,count(distinct c.userid,c.teacherid,c.subject) order_num,count(distinct l.userid) person_num "
                                  ." from %s l left join %s t on l.teacherid = t.teacherid"
                                  ." left join %s tl on (tl.phone =t.phone and tl.status=1 and tl.subject = t.subject)"
                                  ." left join %s m on m.account = tl.account"
                                  ." left join %s c on "
    ...
    public function get_test_person_num_list_by_subject( $start_time,$end_time){
        ...
        $sql = $this->gen_sql_new("select count(distinct l.userid) person_num,count(distinct l.lessonid) lesson_num,l.subject "
                                  ." ,count(distinct c.userid,c.teacherid,c.subject) have_order"
                                  ." from %s l "
                                  ." left join %s tss on tss.lessonid = l.lessonid"
                                  ." left join %s tq on tq.require_id = tss.require_id"
