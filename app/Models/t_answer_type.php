...
use \App\Enums as E;
class t_answer_type extends \App\Models\Zgen\z_t_answer_type
{
    ...
    public function answer_type_list($subject,$open_flag){
        if($subject == 0){
            $where_arr=[
                ["subject=%u", $subject, -1],
        ...
        $sql = $this->gen_sql("select * from %s where %s order by answer_type_no,id asc ",                             
                              self::DB_TABLE_NAME,
                              $where_str
        );
