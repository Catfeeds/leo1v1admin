...
class t_teacher_lecture_appointment_info extends \App\Models\Zgen\z_t_teacher_lecture_appointment_info
{
    ...
    public function get_no_call_all_info(){
        $sql = $this->gen_sql_new("select id,phone from %s a left join %s l on a.phone = l.phone and l.status <>4 and l.add_time=(select max(add_time) from %s where phone = l.phone and status <>4 )"
