...
class t_lesson_info_b2 extends \App\Models\Zgen\z_t_lesson_info
{
    ...
    public function get_teacher_first_test_lesson($page_info,$start_time,$end_time,$subject,$teacherid,$record_flag,$tea_subject=-1){
        ...
        $this->where_arr_add_time_range($where_arr,"l.lesson_start",$start_time,$end_time);
        $sql = $this->gen_sql_new("select l.teacherid,t.realname,l.lessonid,l.lesson_start,l.subject,t.grade_start,t.grade_end,t.grade_part_ex,tr.id,tr.acc,tr.record_info,tr.add_time,l.grade,tr.lesson_invalid_flag  "
                                  ." from %s l left join %s t on l.teacherid = t.teacherid"
    ...
    public function get_teacher_test_lesson_info_for_jy($start_time,$end_time){
        ...
        $this->where_arr_add_time_range($where_arr,"l.lesson_start",$start_time,$end_time);
        $sql = $this->gen_sql_new("select l.teacherid,cur_require_adminid,count(l.lessonid) lesson_count,sum(tss.success_flag in (0,1) and l.lesson_user_online_status =1) suc_count,m.account,m.create_time,sum(if(o.orderid >0,1,0)) order_count,sum(o.price) all_price "
                                  ." from %s l left join %s tss on l.lessonid=tss.lessonid"
                                  ." left join %s t on t.teacherid = l.teacherid"
