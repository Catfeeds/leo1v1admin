...
class t_lesson_info_b3 extends \App\Models\Zgen\z_t_lesson_info{
    ...
    public function get_teacher_lesson_info($teacherid,$start_time,$end_time,$qz_tea_arr=[],$flag=true){
        $where_arr=[
            "t.is_test_user = 0",
            "l.lesson_del_flag = 0",
            "l.lesson_type <1000",
     ...
     * @function 获取平台老师课耗总数
     */
    public function get_teacher_list($start_time,$end_time,$full_flag=0,$fulltime_teacher_type=-1){
        $where_arr = [
            " t.is_quit=0 ",
            " t.is_test_user =0",
            "l.confirm_flag in (0,1,3)",
    ...
    public function get_first_user_subject_tea($userid,$subject){
        ...
        $sql = $this->gen_sql_new("select teacherid from %s l where %s and not exists (select 1 from %s where userid = l.userid and teacherid = l.teacherid and  lesson_del_flag=0 and confirm_flag<2 and lesson_type in (0,1,3) and lesson_start>0 and lesson_status>1 and lesson_start >l.lesson_start)",
                                  self::DB_TABLE_NAME,
                                  $where_arr,
                                  self::DB_TABLE_NAME
        );
    ...
    public function get_lesson_list_by_teacherid($teacherid, $start_time, $end_time) {
        ...
        $sql = $this->gen_sql_new("select lesson_start,lesson_end,lesson_count,grade,level,teacher_type,teacher_money_type,already_lesson_count,competition_flag from %s where %s",
                                  self::DB_TABLE_NAME,
                                  $where
        );
        return $this->main_get_list($sql);
    ...
    public function get_level_for_teacherid($teacherid, $start_time, $end_time) {
        $this->gen_sql_new("select level from t_lesson_info where teacherid=516655 and lesson_start >= $start_time and lesson_start < $end_time group by level", self::DB_TABLE_NAME);
        return $this->main_get_list($sql);
    }
}
