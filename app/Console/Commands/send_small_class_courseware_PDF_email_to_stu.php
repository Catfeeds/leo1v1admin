...
class send_small_class_courseware_PDF_email_to_stu extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:send_small_class_courseware_PDF_email_to_stu';
     ...
     * @var string
     */
    protected $description = "课堂结束发送PDF讲义给小班课学生";
    ...
    public function handle()
    {
        ...
        $end_time   = time();
        $lesson_info = $task->t_lesson_info->get_small_class_lesson_info_for_send_email($start_time,$end_time);
        if(!empty($lesson_info)){
            foreach($lesson_info as $item){         
                ...
                #echo $tea_cw_url_ex;
                if(!empty($tea_cw_url) || $work_status >= 1){
                    foreach($stu_info as $val){
                        if(!empty($val['stu_email'])){
                            if(empty($tea_cw_url)){
                                dispatch( new \App\Jobs\SendEmail(
                                    $val['stu_email'],$lesson_start."-".$lesson_end."小班课课堂讲义",
                                    "<div style=\"text-indent:2em;margin-\">家长您好!</div><br><div style=\"text-indent:2em\">以下是您孩子本次课的课程讲义和课后作业,请及时下载</div><br><div style=\"text-indent:2em\">本邮件由\"上海升学帮\"发送,".
                                    "若想获得更加便捷和专业的服务</div><br><div style=\"text-indent:2em\">请务必下载\"上海升学帮\"关注孩子点滴成长!</div><br><div style=\"color:red;text-indent:2em\">".
