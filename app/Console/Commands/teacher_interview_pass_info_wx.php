...
class teacher_interview_pass_info_wx extends Command
{
 ...
 请赶紧对新入职老师组织入职培训和学科培训,期待通过培训打造强大稳定的在线".$subject_str."教学Team","http://admin.leo1v1.com/tongji_ss/teacher_interview_info_tongji?order_by_str=pass_per%20asc&date_type=null&opt_date_type=0&start_time=".$start."&end_time=".$end."&subject=".$subject);
 }
        }
        ...
        foreach($list as $k=>&$val){
            ...
            $task->t_manager_info->send_wx_todo_msg_by_adminid ($val["uid"],"理优面试组","新老师入职情况汇总",$val["account"]."老师你好,你上周共面试".$val["all_count"]."位老师,入职".@$video_succ_list[$k]["all_count"]."位,通过率".$pass_per_account.",平均审核时长".$account_ave_time."天,所有学科平均面试通过率为".$ave_per.",所有学科平均审核时长".$ave_time."天
              请赶紧对新入职老师组织入职培训和学科培训,期待通过培训打造强大稳定的在线教学Team","http://admin.leo1v1.com/tongji_ss/teacher_interview_info_tongji?order_by_str=pass_per%20asc&date_type=null&opt_date_type=0&start_time=".$start."&end_time=".$end."&teacher_account=".$val["teacherid"]);
