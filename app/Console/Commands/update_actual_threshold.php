    ...
    public function update_send_wx_flag($id,$flag,$threshold_max,$threshold_min,$count_call,$count_no_called){
        ...
        foreach($account_arr as $account){
            $this->task->t_manager_info->send_template_msg(
                ...
                $url='http://p.admin.leo1v1.com/tongji_ex/threshold_detail?start_time='.$start_time.'&end_time='.$end_time.'&r_report_time='.time().'&r_color='.$color.'&r_threshold_line='.$threshold.'&r_count_call='.$count_call.'&r_count_no_called'.$count_no_called);
        }
    }
    ...
    public function send_wx_daily($rate,$time,$start_time,$end_time,$count_call,$count_no_called){
        ...
        foreach($account_arr as $account){
            $this->task->t_manager_info->send_template_msg(
                ...
                $url='http://p.admin.leo1v1.com/tongji_ex/threshold_detail?start_time='.$start_time.'&end_time='.$end_time.'&r_threshold_max'.$threshold_max.'&r_threshold_min'.$threshold_min.'&count_y'.$count_y.'&count_r'.$count_r.'&r_rate'.$rate.'&r_count_call'.$count_call.'&r_count_no_called'.$r_count_no_called);
        }
    }
}
