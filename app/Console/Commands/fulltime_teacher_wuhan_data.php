...
class fulltime_teacher_wuhan_data extends Command
{
    ...
    public function handle()
    {
        ...
        if($arrive_through>0){
            $second_through_per = round(100* $second_through_num_total/$arrive_through,2);
            $second_through_per .= '%('. $second_through_num_total.'/'.$arrive_through.')';
        }else{
        ...
        foreach($arr as $v){
            $task->t_manager_info->send_wx_todo_msg_by_adminid (349,"武汉全职老师面试数据","武汉全职老师面试数据","成功注册人数:".$apply_num.",一面到面人数:".$arrive_num.",一面通过人数:".$arrive_through.",二面通过人数:".$second_through.",入职人数:".$enter_num.",离职人数:".$leave_num.",一面到面率:".$arrive_num_per.",一面通过率:".$arrive_through_per.",录用率:".$second_through_per.",入职率:".$enter_num_per.",离职率:".$leave_per,"");
