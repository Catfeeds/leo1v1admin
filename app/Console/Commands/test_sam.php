        ...
        */
        ...
        foreach ($time as $key => $value) {
            ...
            foreach ($ret_info as $kkey => &$vvalue) {
                ...
                $vvalue['phone_location'] = \App\Helper\Utils::phone_location_to_province($vvalue['phone_location']);
                echo $task->cache_get_teacher_nick($vvalue["teacherid"]).' '.$vvalue['grade_start'].'~'.$vvalue['grade_end'].' '.$vvalue['phone_location'].' '.E\Esubject::get_desc($vvalue['subject']).' '.E\Esubject::get_desc('second_subject').PHP_EOL;
            }
            exit;
            //dd($ret_info);
            //var_dump(date("Y-m-d",$start_time), date("Y-m-d",$end_time));
