...
class Wechat extends \LaneWeChat\Core\Wechat   {
    ...
    public function __construct($token, $debug = FALSE) {
        ...
        \App\Helper\Utils::logger("WX XXLLX :".$data);
        $xml = (array) simplexml_load_string( $data, 'SimpleXMLElement', LIBXML_NOCDATA);
        //将数组键名转换为小写
        \App\Helper\Utils::logger("WX json: ".  json_encode( $xml));
        $this->request = array_change_key_case($xml, CASE_LOWER);
        \App\Helper\Utils::logger("xjrequst: ".  json_encode( $this->request ));
    }
     ...
     */
    public function run() {
        return  \App\Wx\Parent\WechatRequest::switchType($this->request);
    }
};