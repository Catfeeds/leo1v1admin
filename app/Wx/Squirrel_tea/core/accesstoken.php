...
class AccessToken  {
    ...
    public static function getAccessToken(){
        ...
        if(\Squirrel_tea\Core\Environment::isSae(@$_SERVER['HTTP_APPNAME_SST'],@$_SERVER['HTTP_ACCESSKEY_SST'])){
            return self::_getSae();
        }


        $wx= new \App\Helper\Wx(WECHAT_APPID_SST,WECHAT_APPSECRET_SST) ;
        return $wx->get_wx_token(WECHAT_APPID_SST,WECHAT_APPSECRET_SST);
        /*
    ...
    private static function _getAccessToken(){
        $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.WECHAT_APPID_SST.'&secret='.WECHAT_APPSECRET_SST;
        ...
        $f = fopen('/tmp/suqirrel_tea_access_token', 'w+');
    ...
    private static function _checkAccessToken(){
        ...
        $data = @file_get_contents('/tmp/suqirrel_tea_access_token');
    ...
    */
    private static function _getFromFile(){
        if(self::_existsToken()){
            ...
            $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.WECHAT_APPID_SST.'&secret='.WECHAT_APPSECRET_SST; $accessToken = Curl::callWebServer($url, '', 'GET');
            if(!isset($accessToken['access_token'])){
                return Msg::returnErrMsg(MsgConstant::ERROR_GET_ACCESS_TOKEN, '获取ACCESS_TOKEN失败');
            }
            file_put_contents('token.txt', $accessToken['access_token']);
