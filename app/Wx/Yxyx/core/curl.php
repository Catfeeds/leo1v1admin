...
class Curl {
    ...
    public static function callWebServer($queryUrl, $param='', $method='get', $is_json=true, $is_urlcode=true) {
        ...
        if ($method == 'get') {
            ...
            \App\Helper\Utils::logger("curlwx1".$ret);
    ...
    private static function _httpGet($url, $query=array()) {
        ...
        \App\Helper\Utils::logger('urlwx1'.$url);
    ...
    private static function _httpPost($url, $query=array(), $is_urlcode=true) {
        ...
        \App\Helper\Utils::logger('postwx'.json_encode($query));
    ...
    private static function _execute() {
        ...
        \App\Helper\Utils::logger('errorwx1'.$response);

        if ($errno > 0) {
            \App\Helper\Utils::logger('errorinfo');
    ...
    public static function addFile($filename){
        ...
        $addfilewx = class_exists('\CURLFile') ? new \CURLFile($filename) : '@'.$filename;

        \App\Helper\Utils::logger('addfilewx1'.class_exists('\CURLFile'));
