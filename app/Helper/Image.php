...
class Image
{
    ...
    */
    ...
    /**
       ...
       * @param string $mode  类型, 4 中文（暂不支持）
       * @param string $type 图像格式
       ...
       * @throws ThinkExecption
       +----------------------------------------------------------
    */
    static function buildImageVerify($length = 4, $mode = 1, $case=false, $type = 'png', $width = 48, $height = 22, $verifyName = 'verify' )
    {
        $randval = static::build_verify($length, $mode);
        // 默认忽略大小写
        $value= ($case ? $randval : strtolower($randval));
        session(
            [ $verifyName => $value ]
        );
    ...
    */
    ...
    {
        return static::rand_string($length, $mode);
    }
