...
class wx_parent_api extends Controller
{
    ...
    **/
    ...
    public function set_modify_lesson_time_by_parent(){ // 家长设置上课时间段
    ...
    public function get_modify_stauts_by_parent(){// 家长提交申请推送后[详情]
        $lessonid = $this->get_in_int_val('lessonid');

        $lesson_time_arr = [];
        $lesson_time     = $this->t_lesson_info_b2->get_lesson_time($lessonid);

        $lesson_time_arr['lesson_start'] = date('Y年m月d日 H:i:s',$lesson_time[0]['lesson_start']);
        ...
        if($lesson_modify_arr['parent_modify_time'] && ($lesson_modify_arr['is_modify_time_flag'] == 0)  ){
    ...
    public function get_modify_lesson_time_by_teacher(){ // 老师 点击家长调课 推送详情
        $lessonid = $this->get_in_int_val('lessonid');
    ...
    public function set_lesson_time_by_teacher(){// 老师同意时间调整 并设置了自己的时间
        $lessonid        = $this->get_in_int_val('lessonid');
        ...
        $lesson_old_start_date = date('m月d日 H:i:s',$lesson_old_start_date);
        $original_lesson_time = $lesson_old_start.','.$lesson_old_end;

        $lesson_time_arr = explode(',',$lesson_time_str);

        $lesson_new_start_date = date('m月d日 H:i:s',$lesson_time_arr[0]);
        ...
        if($ret1 && $ret2){
            ...
            $data['first']      = " $teacher_nick 老师您好,您于{".$lesson_old_start_date."的".$lesson_name."},已调整至{".$lesson_new_start_date."} ";
            ...
            $data_parent = [
                'first' => "$stu_nick 的家长您好,您的调课申请已经得到 $teacher_nick 老师的同意,{ $lesson_old_start_date }已调整至{ $lesson_new_start_date }",
            ...
            $data_parent = [
                'first'    => "$stu_nick 的家长的调课申请,已经得到{ $teacher_nick }老师的同意,{ $lesson_old_start_date }已经调整至{ $lesson_new_start_date  }",
                'keyword1' => "$lesson_name",
                'keyword2' => "$lesson_new_time",
                'keyword3' => "$stu_nick",
                'remark'   => "请注意调整您的时间安排!"
    ...
    public function teacher_keep_lesson_time(){  // 老师维持原有时间 [提交原因]
        $lessonid = $this->get_in_int_val('lessonid');
        ...
        if($ret){
            ...
            $data_parent = [
                'first'    => "您的学生{ $stu_nick }的家长申请修改{ $lesson_start_date }上课时间被{}老师拒绝!",
                'keyword1' => "调课申请被拒绝",
    ...
    public function jiaowu_deal_modify(){ // 教务处理 [老师保持时间] 推送 家长 老师 销售 咨询 教务
        ...
        $teacher_url = ''; //待定
        $template_id_teacher  = "J57C9QLB-K3SeKgIwdvBMz1RfjUinhwWsN3lEM-Xo5o";
        $data['first']      = "  $teacher_nick 老师您好,$stu_nick 的家长发起的调课申请经过协商已做出修改, { $lesson_old_time 的 $lesson_name } 已调整至{".date('m月d日',$lesson_time_arr[0])."}";
        $data['keyword1']   = " { $lesson_name }";
        $data['keyword2']   = "$day_time";
        $data['keyword3']   = " $stu_nick";
        $data['remark']     = "请注意调整后的时间,如有问题可进行申诉,我们将尽快处理,感谢老师的支持!";
        \App\Helper\Utils::send_teacher_msg_for_wx($teacher_wx_openid,$template_id_teacher, $data,$teacher_url);
        ...
        $parent_template_id      = 'Wch1WZWbJvIckNJ8kA9r7v72nZeXlHM2cGFNLevfAQI';
        $data_parent = [
            'first' => "$stu_nick 的家长您好, 您的调课申请经过协商已经做出修改, { $lesson_old_time 的 $lesson_name },已调整至{ ".date('m月d日',$lesson_time_arr[0])." }",
            'keyword1' =>"$lesson_name",
            'keyword2' => "$lesson_new_time",
            'keyword3' => "$stu_nick",
            'remark'   => "请注意调整后的时间,如有问题可进行申诉,我们会尽快处理,感谢家长的支持!"
        ];
        $url_parent = '';
        $wx = new \App\Helper\Wx();
        $wx->send_template_msg($parent_wx_openid, $parent_template_id, $data_parent, $url_parent);
        ...
        $data_parent = [
            'first'    => "由 $stu_nick 的家长发起的调课申请已经处理完成{ $lesson_old_time 的 $lesson_name },已调整至{ ".date('m月d日',$lesson_time_arr[0])." }",
            'keyword1' => "$lesson_name",
            'keyword2' => "$lesson_new_time",
            'keyword3' => "$stu_nick",
            'remark'   => "请注意调整您的时间安排!"
    ...
    public function teacher_modify_lesson_time(){// 老师更换时间
        $lessonid = $this->get_in_int_val('lessonid');
    ...
    public function set_modify_lesson_time_by_teacher(){ //老师更换时间
        $lessonid = $this->get_in_int_val('lessonid');
    ...
    public function set_lesson_time_by_parent(){ // 家长同意老师申请的时间
        $lessonid = $this->get_in_int_val('lessonid');
        $lesson_time_str = $this->get_in_str_val('lesson_time_str');
    ...
    public function send_wx_msg_by_agree($lessonid,$is_teacher_agree){ // 家长或老师 同意了时间后 发送微信推送
        ...
        if($is_teacher_agree == 1){ // 家长同意
            ...
            $data_leo['first'] = "{ $teacher_nick } 老师申请调整{ $stu_nick }的家长发起的调课申请,已获得{ $stu_nick }家长的同意,原课程时间{},最终时间调整至{ $lesson_new_time }";
