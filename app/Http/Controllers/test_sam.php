...
class test_sam  extends Controller
{
    ...
    public function  tt(){
        ...
        $insert_data = [
          ...
          "month_kpi_per"           => $arr['month_kpi_per']*100,//A7-月KPI完整率(月初至今)
          ...
          "lesson_consume"          => $arr['lesson_consume']*100,//B5-课时消耗实际数量
          "teacher_leave"           => $arr['teacher_leave']*100,//B6-老师请假课时
          "student_leave"           => $arr['student_leave']*100,//B7-学生请假课时
          "other_leave"             => $arr['other_leave']*100,  //B8-其他原因未上课时

          "student_arrive"          => $arr['student_arrive'],   //学生到课数量
          "lesson_plan"             => $arr['lesson_plan'],      //排课数量
          "student_arrive_per"      => $arr['student_arrive_per']*100,//B10-学生到课率
          "lesson_income"           => $arr['lesson_income']*100,//B11-课时收入
          ...
          "total_renew"             => $arr['total_renew']*100,  //C5-续费金额
          "renew_num_per"           => $arr['renew_num_per']*100,//C6-续费平均单笔
          "renew_per"               => $arr['renew_per']*100,    //C7-月续费率
          "finish_renew_per"        => $arr['finish_renew_per']*100,//C8-月预警续费率

          "tranfer_phone_num"       => $arr['tranfer_phone_num'],//D1-转介绍至CC例子量
          "tranfer_total_price"     => $arr['tranfer_total_price']*100, //D2-转介绍至CC例子签单金额
          "tranfer_total_num"       => $arr['tranfer_total_num'], //D3-转介绍至CC例子签单量
          "tranfer_success_per"     => $arr['tranfer_success_per']*100,//D4-月转介绍至CC签单率
          "tranfer_num"             => $arr['tranfer_num'],       //D5-转介绍成单数量
          "total_tranfer"           => $arr['total_tranfer']*100, //D6-转介绍总金额
          "tranfer_num_per"         => $arr['tranfer_num_per']*100,//D7- 转介绍平均单笔
          ...
          "kk_success_per"          => $arr['kk_success_per'],    //E5-月扩课成功率
