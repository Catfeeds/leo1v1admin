...
//没有小学
    ...
    public function get_order_new() {
        ...
        $sql = 'select o.order_time,m.account,m.become_member_time,o.price,o.check_money_time from db_weiyi.t_order_info o '
            .'left join db_weiyi_admin.t_manager_info m on m.account=o.sys_operator left join db_weiyi_admin.t_admin_group_user u '
            .'on u.adminid=m.uid  where  m.del_flag=0 and m.account_role=2 and o.contract_status>0 and o.contract_type =0 '
            .'and m.del_flag=0 and o.order_time>=1512057600 and o.order_time<1514736000';
        $ret    = $this->t_grab_lesson_link_info->get_info_test($sql);
        $th_arr = ['cc', '入职时间', '金额', '下单时间', '财务确认时间'];
        $s      = $this->table_start($th_arr);
    ...
    public function get_cc(){
        ...
        $sql = 'select m.uid,count( tq.id ) phone_count, sun( if(tq.is_called_phone==1,0,1) ) no_acc, succ_count, '
             .'count(tr.userid) yue_count, count(o.orderid) order_count, count(f.orderid) refund_count '
             ." from db_weiyi_admin.t_manager_info m "
             ." left join db_weiyi_admin.t_tq_call_info tq on tq.uid=m.uid and tq.id>0 and tq.start_time >=$start and tq.start_time< $end"
             // ." left join on db_weiyi.t_test_lesson_subject_require tr on tr.require_adminid=m.uid "
             // ." left join on db_weiyi.t_test_lesson_subject ts on ts.test_lesson_subject_id=tr.test_lesson_subject_id "
             // ." left join db_weiyi.t_order_info o on o.sys_operator=m.account and o.order_time>=$start and o.order_time<$end"
             // ." left join db_weiyi.t_order_info oo on o.sys_operator=m.account "
             // ." left join db_weiyi.t_order_refund f on f.orderid=oo.orderid and f.apply_time>=$start and f.apply_time<$end"
             // ." left join db_weiyi_admin.t_seller_edit_log s on s.uid=m.uid and create_time>=$start and create_time<$end"
             // ." left join db_weiyi_admin.t_seller_edit_log ss on ss.uid=m.uid and create_time>=$start and create_time<$end"
             ." where m.account_role=2";
        return $sql;
    ...
    public function get_order_info(){
        ...
        $gra = [];
        $sub = [];
        $cou = [];// 0-90 90-180 180-270 270-360 360-480 480-
        $count = count($ret);
        foreach($ret as $v){
