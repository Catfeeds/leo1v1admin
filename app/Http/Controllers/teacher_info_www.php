...
class teacher_info_www extends TeaController
{
    ...
    public function teacher_apply_list() {
        ...
        $lesson_info = $this->t_lesson_info->get_lesson_info_by_lessonid('lessonid,assistantid,lesson_type,lesson_name,train_type',array_column($ret_info['list'],'lessonid'));
        if($lesson_info){
            foreach($ret_info['list'] as &$item){
                foreach($lesson_info as $key=>$info){
                    if($item['lessonid'] == $info['lessonid']){
                        ...
                        if($info['lesson_type']<1000){
                            $item['ass_nick'] = $this->cache_get_assistant_nick($info['assistantid']);
                        }elseif($info['lesson_type']==1100 && $info['train_type']==4){
                            $item['ass_nick']  = "沈老师";
                            $item['ass_phone'] = "15214368896";
                        }
                    }else{
    ...
    public function teacher_apply_add() {
        ...
        $this->send_wx_apply($cc_id,$question_type,$teacherid,$lesson_time,$question_content,$stu_nick);

        return $this->output_succ();
    }

    public function send_wx_apply($cc_id,$question_type,$teacherid,$lesson_time,$question_content,$stu_nick){
        $account = $this->t_manager_info->get_account_by_uid($cc_id);
        ...
        $msg = "上课时间：".$lesson_time.";问题描述：".$question_content.";日期：".date("Y-m-d H:i:s",time()).";为保证试听质量，请您尽快回访处理。";
        $send_wx_data = [
            "account"    => urlencode($account),
            "from_user"  => urlencode($question_type_str),
            "header_msg" => urlencode($teacher_nick."老师对".$stu_nick."同学的试听课需求有如下疑问："),
            "msg"        => urlencode($msg),
            "url"        => $wx_url,
        ];
