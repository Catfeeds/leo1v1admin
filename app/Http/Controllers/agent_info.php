...
class agent_info extends Controller
{
    ...
    public function get_agent_group_list() {
        ...
        $ret_info = $this->t_agent_group->get_agent_group_list($agentid);
        return $this->pageView(__METHOD__,$ret_info);
    }

    public function agent_apply_list() {
        $agentid = $this->get_login_agent();
        $page_info = $this->get_in_page_info();
        $page_num  = $this->get_in_page_num();
        $ret_info  = [];
        $ret_info  = $this->t_agent_apply->get_agent_apply_list($agentid,$page_num);
    ...
    public function agent_apply_add() {
        $agentid          = $this->get_login_agent();
        ...
        $this->send_wx_apply($adminid,$question_type,$agentid,$lesson_time,$question_content,$stu_nick);

        return $this->output_succ();
    }

    public function send_wx_apply($adminid,$question_type,$agentid,$lesson_time,$question_content,$stu_nick)
    {
        $account           = $this->t_manager_info->get_account_by_uid($adminid);
        $question_type_str = $this->get_question_type_str($question_type);
        $wx_url            = "http://admin.yb1v1.com/agent_apply/agent_apply_list_one";
        $agent_nick      = $this->t_agent_info->get_nick($agentid);
        ...
        $send_wx_data = [
            ...
            "header_msg" => urlencode($agent_nick."老师对".$stu_nick."同学的试听课需求有如下疑问："),  //主题
            "msg"        => $msg,                                                                        //内容
            "url"        => $wx_url,
            "desc"       => $desc,                                                                        //底部内容
        ];
    ...
    public function agent_apply_edit() {
        $id = $this->get_in_id();
        $agent_flag=$this->get_in_int_val('agent_flag',0);
        if($agent_flag){
            $this->t_agent_apply->field_update_list($id,[
    ...
    public function agent_apply_del() {
        $id=$this->get_in_id();
        $this->t_agent_apply->row_delete($id);
        return $this->output_succ();
    }
    ...
    public function get_lesson_list() {
        ...
        $ret_info=$this->t_lesson_info_b2->get_agent_lesson_list($agentid,$start_time,$end_time,$lesson_type_in_str, $page_num);
        foreach($ret_info["list"] as &$item){
    ...
    {
        $agentid   = $this->get_login_agent();
        $common_lesson_config= $this->t_agent_freetime_for_week->get_common_lesson_config($agentid);
        $date=\App\Helper\Utils::get_week_range(time(NULL),1);
        $stime=$date["sdate"];
        foreach ( $common_lesson_config as &$item ) {
            ...
            $item["agent"]       = $this->cache_get_agent_nick($agentid);
        }
    ...
    public function otp_common_config()
    {
        ...
        $agent_in = $this->t_agent_freetime_for_week->check_userid($agentid);
        if($agent_in!= 1){
            $this->t_agent_freetime_for_week->add_regular_course($agentid,$tea_type,$freetime);
        }
        $common_lesson_config = $this->t_agent_freetime_for_week->get_common_lesson_config($agentid);
        ...
        $ret_db = $this->t_agent_freetime_for_week->field_update_list($agentid,[
    ...
    public function get_lesson_time_js() {
        ...
        $lesson_list=$this->t_lesson_info_b2->get_agent_lesson_info($agentid,$start_time,$end_time);
    ...
    function update_agent_student_pdf() {
        $agentid          = $this->get_login_agent();
        ...
        $now = time(NULL);
        $db_agentid = $this->t_lesson_info_b2->get_agentid($lessonid);
        if ($db_agentid != $agentid) {
        ...
        if($tea_cw_pic_flag==1 && $old_tea_cw_url!=$tea_cw_url){
            ...
            $post_url  = $admin_url."/common_new/notify_gen_lesson_agent_pdf_pic?lessonid=".$lessonid;
    ...
    public function tea_lesson_count_detail_list() {
        ...
        $old_list = $this->t_lesson_info_b2->get_1v1_lesson_list_by_agent($agentid,$studentid,$start_time,$end_time);
        ...
        foreach ($old_list as $row_id=> &$item) {
            ...
            //agent level
            $level=$item["level"];
            $agent_money_type =$item["agent_money_type"];


            $price_class=\App\Config\agent_price_base::get_price_class($agent_money_type,$level);
    ...
    public function agent_all_info(){
        $agentid   = $this->get_login_agent();
        if(!$agentid){
            return $this->output_err("老师登陆信息错误，请退出重新登陆!");
        }
        $tea_info=$this->t_agent_info->get_agent_info_all($agentid);
    ...
    public function set_agent_face(){
        $agentid = $this->get_in_int_val("agentid");
        $face = $this->get_in_str_val("face");
        $domain = config('admin')['qiniu']['public']['url'];
        $face = $domain.'/'.$face;
        $this->t_agent_info->field_update_list($agentid,[
            "face" => $face,
        ]);
        return $this->output_succ();
    }
    ...
    private function get_agent_lesson_money($type,$already_lesson_count){
        $rule_type = \App\Config\agent_rule::$rule_type;
        $reward    = 0;
    ...
    public function agent_lecture_appointment_info(){
        list($start_time,$end_time) = $this->get_in_date_range(date("Y-m-01",time()),0,0,[],3);
        $page_num  = $this->get_in_page_num();
        $agentid = $this->get_login_agent();
        $status    = $this->get_in_int_val("status",-1);

        $phone    = $this->t_agent_info->get_phone($agentid);
        //dd($phone);
        $ret_info = $this->t_agent_lecture_appointment_info->get_all_info_b1($page_num,$start_time,$end_time,$phone,$status);
        //dd($ret_info);
        ...
        $all_info  = $this->t_agent_lecture_appointment_info->get_lecture_count_info($start_time,$end_time,$phone);
        $count_num = [
            "all"      => 0,
            "pass"     => 0,
            "not_pass" => 0,
    ...
    public function course_set_new() {
        ...
        //检查老师一周排课功能是否冻结
        $week_freeze_info = $this->t_agent_info->field_get_list($agentid,"is_freeze,is_week_freeze,week_freeze_time,lesson_hold_flag,is_test_user");
        ...
        //老师需满足培训通过的条件才能排试听课
        $agent_type_train_info = $this->t_agent_info->field_get_list($agentid,"agent_money_type,train_through_new");
        if($agent_type_train_info["agent_money_type"]>3 && $agent_type_train_info["train_through_new"]==0 && $is_test==0){
        ...
        $tt_item['grade']=$grade;
        $agent_subject = $this->t_agent_info->field_get_list($agentid,"subject,second_subject,third_subject,grade_part_ex,second_grade,third_grade,grade_start,grade_end,not_grade,not_grade_limit,limit_plan_lesson_type");

        if($agent_subject['grade_start']==0){
            $check_subject= $this->check_agent_subject_and_grade(
                $subject,$grade,$agent_subject["subject"],$agent_subject["second_subject"],$agent_subject["third_subject"],
                $agent_subject["grade_part_ex"],$agent_subject["second_grade"],$agent_subject["third_grade"],$is_test,
                $agent_subject['not_grade']
            );
        }else{
            $check_subject=$this->check_agent_grade_range_new($tt_item,$agent_subject);
        }
        if($check_subject!=1){
        ...
        //系统限课
        $test_lesson_num = $this->t_lesson_info_b2->get_limit_type_agent_lesson_num($agentid,$lstart,$lend);
        if($test_lesson_num >= $agent_subject["limit_plan_lesson_type"]
           && $is_test==0
           && $agent_subject["limit_plan_lesson_type"] !=0
        ){
            return $this->output_err(
                "您排课受限制,一周限排".$agent_subject["limit_plan_lesson_type"]."节,"
                ."当周您已排".$test_lesson_num."节,不能继续排课!"
            );
        }

        //新入职老师当周限排6节课,其他老师每周限排8节课,一天限排4节课
        $limit_num_info = $this->t_agent_info->field_get_list($agentid,"limit_day_lesson_num,limit_week_lesson_num");
        $ret = $this->t_lesson_info_b2->check_agent_have_test_lesson_pre_week($agentid,$lstart);
        if($ret ==1){
        ...
        $test_lesson_num_day = $this->t_lesson_info_b2->get_limit_type_agent_lesson_num($agentid,$day_start,$day_end);
        ...
        $tea_free=$this->t_lesson_info_b2->check_agent_time_free(
            $agentid,0,$lesson_start,$lesson_end);
        if($tea_free) {
            $error_lessonid=$tea_free["lessonid"];
            return $this->output_err("您有一堂课和此课程冲突，无法排课!");
        }
        $agent_info = $this->t_agent_info->field_get_list($agentid,"agent_money_type,level");

        $this->t_course_order->start_transaction();
        $courseid = $this->t_course_order->add_course_info_new($orderid,$userid,$grade,$subject,100,2,0,1,1,0,$agentid);
        $lessonid = $this->t_lesson_info_b2->add_lesson(
            $courseid,0,$userid,0,2,
            $agentid,0,$lesson_start,$lesson_end,$grade,
            $subject,100,$agent_info["agent_money_type"],$agent_info["level"]
        );
        ...
        $this->t_homework_info->add($courseid,0,$userid,$lessonid,$grade,$subject,$agentid);
        ...
        if (\App\Helper\Utils::check_env_is_release()){
            ...
            $agent_nick = $this->cache_get_agent_nick($agentid);
            ...
            $header_msg = "排课[$phone][$nick] 老师[$agent_nick]上课时间[$lesson_time_str]";
            $data = [
    ...
    public function check_agent_subject_and_grade($subject,$grade,$first_subject,$second_subject,$third_subject,$grade_part_ex,$second_grade,$third_grade,$is_test,$not_grade){
        if($is_test ==0){
    ...
    public function check_agent_grade_range_new($stu_info,$tea_info){
    ...
    public function tea_ref_money_list() {
        ...
        $agentid       = $this->get_in_int_val("agentid",-1);
        $start_time      = strtotime($start_date);
        $end_time        = strtotime("+1 month",$start_time);
        $last_start_time = strtotime("-1 month",$start_time);
        $last_end_time   = strtotime("-1 month",$end_time);
        $uid             = $this->get_login_agent();

        $agent_info     = $this->t_agent_info->get_agent_info_all($uid);
        $agent_ref_type = 0;
        if(!empty($agent_info)){
            if($agent_info['agent_ref_type']>0 && $agent_info['agent_type']==21){
                $agent_ref_type = $agent_info['agent_ref_type'];
            }
        }

        if($agent_ref_type==1){
            $agent_ref_rate = \App\Helper\Config::get_config_2("agent_ref_rate",$agent_ref_type);
        }elseif($agent_ref_type!=0){
            $agent_ref_num  = $this->t_agent_info->get_agent_ref_num($start_time,$agent_ref_type);
            $agent_ref_rate = \App\Helper\Utils::get_agent_ref_rate($agent_ref_num);
        }
        ...
        $check_date = "2017-6-1";
        if($agent_ref_type!=0){
            $teaid_list    = $this->t_agent_info->get_tea_list_by_ref_type($agent_ref_type,$agentid);
            $agentid_str = "";
            foreach($teaid_list as $tea_val){
                if($tea_val['create_time']>strtotime($check_date) && $tea_val['agent_type']<20){
                    continue;
                }
                $tea_id = $tea_val['agentid'];
                $agentid_str .= $tea_id.",";
                if(!isset($agent_money[$tea_id])){
                    $reward_money     = $this->t_agent_money_list->get_agent_reward_money($tea_id,$start_time,$end_time,3);
                    $reward_ref_money = $reward_money*$agent_ref_rate;
                    $agent_money[$tea_id]['money']             = $reward_money;
                    $agent_money[$tea_id]['cost']              = 0;
                    $agent_money[$tea_id]['agent_ref_money'] = $reward_ref_money;
                    $agent_money[$tea_id]['realname']          = mb_substr($tea_val['realname'],0,1,"utf8")."老师";
                    $agent_money[$tea_id]['reference_name']    = $tea_val['reference_name'];
                    $agent_money[$tea_id]['phone']             = substr($tea_val['phone'],0,3)."****".substr($tea_val['phone'],7,11);
                    $all_money['agent_money']                 += $reward_money;
                    $all_money['all_money']                     += $reward_ref_money;
                }
            }

            $tea_list = $this->t_lesson_info_b2->get_tea_month_list($start_time,$end_time,trim($agentid_str,","));
            foreach($tea_list as &$val){
                $tid          = $val['agentid'];
                $lesson_count = $val['lesson_count']/100;
                if(!isset($already_lesson_count[$tid])){
                    $already_lesson_count[$tid] =
                        $this->t_lesson_info_b2->get_agent_last_month_lesson_count($tid,$last_start_time,$last_end_time);
                }

                $val['already_lesson_count'] = $already_lesson_count[$tid];
                $this->get_lesson_cost_info($val);
                $reward = \App\Helper\Utils::get_agent_lesson_money($val['type'],$val['already_lesson_count']);
                $money  = ($val['money']+$reward)*$lesson_count-$val['lesson_cost'];
                $agent_ref_money = $money*$agent_ref_rate;

                $agent_money[$tid]['money']             += $money;
                $agent_money[$tid]['agent_ref_money'] += $agent_ref_money;
                $all_money['agent_money']               += $money;
                $all_money['all_money']                   += $agent_ref_money;
            }
        }else{
              $show_agent_money = [];
             return $this->pageView(__METHOD__,$show_agent_money,[
                    "agent_ref_type" => 1,
                    "all_money"        => 0,
                    "uid"              => $uid,
                ]);
        ...
        $order_money_list   = [];
        $show_agent_money = [];
        foreach($agent_money as $money_val){
            if($money_val['money']>0){
                $show_agent_money[] = $money_val;
                $order_money_list[]   = $money_val['money'];
            }
        }

        array_multisort($order_money_list,SORT_DESC,$show_agent_money);
        $all_money['all_money'] = round($all_money['all_money'],2);

        $show_agent_money = \App\Helper\Utils::list_to_page_info($show_agent_money);
    ...
    public function add_complaint_info(){
        ...
        $last_info_arr = $this->t_complaint_info->get_last_msg($agentid);
        ...
        if ($ret_info_qc) {
            ...
            $opt_nick      = $this->cache_get_agent_nick($agentid);
            ...
            foreach($qc_openid_arr as $qc_item){
                $this->send_agent_msg($qc_item,$opt_nick,$msg);
            }
            ...
            **/
            $url = '';
            $agent_openid   = $this->t_agent_info->get_wx_openid_by_agentid($agentid);

            $template_id_agent  = "kvkJPCc9t5LDc8sl0ll0imEWK7IGD1NrFKAiVSMwGwc";
            ...
            $ret = $wx->send_template_msg($agent_openid,$template_id_agent,$data ,$url);
            \App\Helper\Utils::logger("teac_msg:$ret,agent_openid:$agent_openid");

            return $this->output_succ();
        }
    }

    public function send_agent_msg($qc_account,$opt_nick,$msg){
        $from_user  = "课程投诉";
    ...
    public function  file_store()   {
        ...
        $ret_list=$store->list_dir($agentid, $dir);
    ...
    public function get_upload_token() {
        ...
        $dir       = $this->get_in_str_val("dir");
        $agentid = $this->get_login_agent();
        $pre_dir   = $store->get_dir($agentid,$dir );
    ...
    public function get_download_url() {
        ...
        $store=new \App\FileStore\file_store_tea();
        $auth=$store->get_auth();
        $file_path = $store->get_file_path($agentid,$file_path);
    ...
    public function get_share_link() {
        ...
        $arr=[
            ...
            "end_time" => $end_time ,
            "md5_sum" => substr( md5("$agentid:$share_path:$create_time,$end_time"), 0, 16)
        ];
    ...
    public function file_store_rename() {
        ...
        $store=new \App\FileStore\file_store_tea();
        $store->rename_file($agentid,$old_path,$new_name);
        return $this->output_succ();
    }
    ...
    public  function file_share() {
        ...
        if( $md5_sum!== substr( md5("$agentid:$share_path:$create_time,$end_time"), 0, 16)) {
            return $this->error_view([
                "md5 校验失败"
            ]);
        }
        ...
        $ret_list=$store->list_dir($agentid, $obj_dir);
        $list=[];
        foreach ( $ret_list  as $item  ) {
            if (!$item["is_dir"]) {
                \App\Helper\Utils::unixtime2date_for_item($item,"create_time");
    ...
    public function get_agent_basic_info(){
        $agentid = $this->get_login_agent();
        $ret_info  = $this->t_agent_info->get_agent_info_to_agent($agentid);

        foreach ($ret_info['list'] as &$item) {
            E\Esubject::set_item_value_str($item);
            E\Etextbook_type::set_item_value_str($item);
            E\Eidentity::set_item_value_str($item);
            E\Eagent_ref_type::set_item_value_str($item);
            ...
            \App\Helper\Utils::unixtime2date_for_item($item,"create_time");
            $item['agent_title'] = \App\Helper\Utils::get_agent_level_str($item);
            ...
            $item['agent_tags_arr'] = explode(',',$item['agent_tags']);
            $item['tags_flag'] = count($item['agent_tags_arr']);
            //判断完整度
    ...
    public function edit_agent_info(){
        ...
        $ret_info = $this->t_agent_info->field_update_list($agentid,[
            "gender"        => $gender,
            "birth"         => $birth,
            "work_year"     => $work_year,
            "school"        => $school,
    ...
    public function edit_agent_bank_info(){
        $agentid     = $this->get_login_agent();
    ...
    public function get_agent_money_info(){
        $agentid = $this->get_login_agent();
        if(!$agentid){
        ...
        $first_lesson_time = $this->t_lesson_info_b3->get_first_lesson_time($agentid);
        ...
        $simple_info = $this->t_agent_info->get_agent_info($agentid);
        $agent_money_flag = $simple_info['agent_money_flag'];
        $agent_type       = $simple_info['agent_type'];
        $transfer_agentid = $simple_info['transfer_agentid'];
        $agent_level_str = \App\Helper\Utils::get_agent_level_str($simple_info);

        $lesson_list = $this->t_lesson_info->get_lesson_list_for_wages($agentid,$begin_time,$end_time);
        $list      = [];
        $check_num = [];
        $already_lesson_count_list = [];
        foreach($lesson_list as $val){
            $check_type   = \App\Helper\Utils::check_agent_money_type($val['agent_money_type'],$agent_type);
            $lesson_count = $val['confirm_flag']!=2?($val['lesson_count']/100):0;
            $month_key    = date("Y-m",$val['lesson_start']);
            $simple_info['level'] = $val['level'];
            $level_str = \App\Helper\Utils::get_agent_level_str($simple_info);
            ...
            $key = "already_lesson_count_".$month_key."_".$agentid;
            if(!isset($already_lesson_count_list[$key])){
                ...
                if($last_lesson_count === null){
                    ...
                    $last_lesson_count = $this->get_already_lesson_count(
                        $last_start_time,$last_end_time,$agentid,$val['agent_money_type']
                    );
            ...
            if($val['lesson_type'] != 2){
                $val['money']       = \App\Helper\Utils::get_agent_base_money($agentid,$val);
                $val['lesson_base'] = $val['money']*$lesson_count;
                $lesson_reward       = \App\Helper\Utils::get_agent_lesson_money($val['type'],$already_lesson_count);
                ...
                $val['lesson_base'] = \App\Helper\Utils::get_trial_base_price(
                    $val['agent_money_type'],$val['agent_type'],$val['lesson_start']
                );
                $lesson_reward       = "0";
        ...
        $reward_list = $this->t_agent_money_list->get_agent_honor_money_list($agentid,$begin_time,$end_time);
        foreach($reward_list as $r_val){
            ...
            case E\Ereward_type::V_6:
                $reward_arr["name"]=$this->cache_get_agent_nick($r_val['money_info']);
        ...
        return $this->pageView(__METHOD__,[],[
            ...
            "agent_level_str" => $agent_level_str
        ]);
    }
    ...
    private function get_lesson_cost_info(&$val,&$check_num){
        ...
        $agent_money = \App\Helper\Config::get_config("agent_money");
        $month_key     = date("Y-m",$val['lesson_start']);
        ...
        if($val['confirm_flag']==2 && $val['deduct_change_class']>0){
            ...
            $lesson_cost = $agent_money['lesson_cost']/100;
    ...
    public function edit_agent_status(){
        $agentid = $this->get_login_agent();
        ...
        $res_info = $this->t_agent_info->field_update_list($agentid, ["need_test_lesson_flag" => $need_test_lesson_flag,]
        );
    ...
    public function update_agent_pdf_info(){
        $agentid = $this->get_login_agent();
        ...
        $res_info = $this->t_agent_info->field_update_list($agentid, [$field  => $url]);
    ...
    public function edit_agent_face() {
        $agentid = $this->get_login_agent();
        $face    = $this->get_in_str_val('face', '');
        $pub_url = config::get_qiniu_public_url()."/";
        $face    = $pub_url.$face;

        $res_info = $this->t_agent_info->field_update_list($agentid, ['face' => $face]);
    ...
    public function get_train_list(){
        ...
        $agentid = $this->get_login_agent();
        $page_info = $this->get_in_page_info();
        $ret_info = $this->t_agent_train_info->get_train_list($page_info,$agentid,$start_time,$end_time,$train_type,$subject,$status);
        //  dd($ret_info);
        foreach( $ret_info["list"] as $key => &$item ) {
            ...
            $this->cache_set_item_agent_nick($item);
            E\Esubject::set_item_value_str($item);
    ...
    public function grab_visit_info(){
        ...
        if ($visitid > 0 & $grabid >0) {//点击抢课，记录信息
            ...
            //修改逻辑，visitid-requireid-agentid三个唯一
            $operationid = $this->t_grab_lesson_link_visit_operation->get_operationid_by_tea_requireid($agentid,$requireid, $visitid);
